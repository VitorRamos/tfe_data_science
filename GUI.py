...
class Graph:
    ...
    def __init__(self,data,master,choice):
        ...
        if self.__choice == 'L':
            ...
            #self.__axis, = self.__fig.plot(, markevery=markers)
        if self.__choice == 'B':    
            self.__axis= self.__fig.add_subplot(111)
    ...
    def updateType(self,choice):
        print(choice)
        print(self.__choice)
        print('data_disp',self.__data_disp)
        self.__axis.clear()
        ...
        if self.__choice == 'B':    
            self.__axis= self.__fig.add_subplot(111)
            print(len(self.__data_disp["ipmi_energy"]))
            N=len(self.__data_disp["ipmi_energy"])
            ind = np.arange(N)
            width = 0.35
            print("Bars")
            ...
            self.__axis.bar(ind,self.__data_disp["ipmi_energy"],width)
        if self.__choice == 'P':
            print('__data_disp',self.__data_disp)
            if not self.__data_disp.empty:
                ...
                min_v= np.argmin(self.__data_disp[self.__axesY])
                print('min_v',min_v)
                print(self.__data_disp.loc[min_v])
